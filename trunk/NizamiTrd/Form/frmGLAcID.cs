...
//
//using NizamTrd.Security.Cls;
using System.Data.SqlClient;
//using NizamTrd.GL.Cls;
//using NizamiTrd.GL.COA;
using NizamiTrd.Class;
using System.Text.RegularExpressions;
using NizamiTrd.StringFun01;
//using NizamTrd.GL.Cmn;
//using NizamTrd.Cmn;
...
namespace NizamiTrd
{
    public partial class frmGLCOA : Form
    {
        ...
        int fcboCityDefaultValue = 0;
        // For city Search Change in LookUp
        bool fOneTable = true;
        ...
        { 
            ...
            fTableName = fMain[2];
            fKeyFieldName = fMain[3];
            fPidField = fMain[4];
            fKeyFieldStr = fMain[5];
            //AddContextMenu();
            //fJoin = INNER JOIN geo_city p ON kt.ac_city_id = p.city_id ;
                 //"INNER JOIN geo_city p ON kt.ac_city_id = p.city_id";
            //fReplaceAble = "ac_city_id,city_title,City ID,City Title";
        ...
        private void AtFormLoad()
        {
            ...
            textaTitle.BorderStyle = BorderStyle.FixedSingle;
            ...
            string lSQl = "select actype_id, actype_title, isDefault from " + "gl_actype";
            //lSQl += " where ";
            //lSQl += clsGVar.LGCY;
            ...
            //lSQl = "select city_id, city_title from " + "geo_city";
            //lSQl += " order by city_Title,ordering";
            //clsFillCombo.FillCombo(
            //    cboCity,
            //    clsGVar.ConString1,
            //    "geo_city" + "," + "city_id" + ",False", lSQl);
            //fcboCityDefaultValue = Convert.ToInt16(cboCity.SelectedValue);
            
        }
        //
        private void RefreshCityCbo()
        {
          ...
          string lSQl = "select city_id, city_title from " + "geo_city";
          lSQl += " order by city_Title,ordering";
          clsFillCombo.FillCombo(
              cboCity,
              clsGVar.ConString1,
        ...
        { 
            ...
            lbtn_Clear.Image = Properties.Resources.BaBa_clear;
        ...
        private void FindRecursive(TreeNode treeNode)
        {
            foreach (TreeNode tn in treeNode.Nodes)
            {
                ...
                //if (tn.Text == textSearchTree.Text)
                {
                    ...
                    tn.Expand();
                    textAlert.Text = "'" + textSearchTree.Text + "'" + " Found. " + now.ToString("T");
                }
                FindRecursive(tn);
            }
        }
        ...
        private void btn_PinParentID_Click(object sender, EventArgs e) 
        {
            if (btn_PinParentID.Text == "&Pin")
            {
                ...
                btn_PinParentID.Image = NizamiTrd.Properties.Resources.tiny_pinned;
                mtextParentID.Enabled = false;
            }
            else
            {
                btn_PinParentID.Text = "&Pin";
                btn_PinParentID.Image = NizamiTrd.Properties.Resources.tiny_pin;
                mtextParentID.Enabled = true;
            }
        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (fParentGroupTran == "True")                  // Note: bit as string = true == "1")
                        {
                            ...
                            textAlert.Text = "Transactional ID is not allowed as parent ID, Select another with 'Group' Ac " + now.ToString("T"); 
                        }
                        else
                        {
                        ...
                        {
                            cboAcType.SelectedIndex = clsSetCombo.Set_ComboBox(cboAcType, Convert.ToInt16(fParentAcType) );
        ...
        private void mtextID_Validating(object sender, CancelEventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string tSQL = string.Empty;
                    tSQL = "select top 1 ";
                    tSQL += "ac_id, ac_title, ac_atitle, ac_st, ordering, ac_level, actype_id,istran, isbudget, ac_pid, ac_city_id, addr_uid ";
                    tSQL += " from " + "gl_ac";
                    tSQL += " where ";
                    //tSQL += clsGVar.LGCY;
                    //tSQL += " and ";
                    ...
                    {
                        fAlreadyExists = true;
                        textAlert.Text = "Modifying Account ID " + mtextID.Text.ToString() + "  " + now.ToString("T");
                        dRow = dtset.Tables[0].Rows[0];
                        // Starting title as 0
                        ...
                        textaTitle.Text = dRow.ItemArray.GetValue(2).ToString();
                        ...
                        fCityID = dRow.ItemArray.GetValue(10).ToString();
                        if (dRow.ItemArray.GetValue(11) != DBNull.Value)
                        {
                          fAddressUID = Convert.ToInt64(dRow.ItemArray.GetValue(11).ToString());
                        }
                        else
                        {
                          fAddressUID = 0;
                        }
                        lblAc_id.Text = fAcID + " / " + fAddressUID.ToString();
                        //
                        cboAcType.SelectedIndex = clsSetCombo.Set_ComboBox( cboAcType, Convert.ToInt16(fAcType) );
                        ...
                        cboCity.SelectedIndex = clsSetCombo.Set_ComboBox(cboCity, Convert.ToInt16(fCityID));
                        //fAddressUID = (dtset.Tables[0].Rows[0]["addr_uid"] == DBNull.Value ? 0 : Convert.ToInt64(dtset.Tables[0].Rows[0]["addr_uid"].ToString()));
                        //lblAc_id.Text = fAcID + " / " + fAddressUID.ToString();
                        //
            ...
            {
                MessageBox.Show("Exception, Validation : " + ex.Message, lblFormTitle.Text.ToString());
            }
        }
        ...
        public void frmGLAcLookUpParent()
        {
            ...
            try
            {
                ...
                frmLookUp sForm = new frmLookUp(
                    "ac_strid",
                    "a.ac_title,a.ac_level,a.ac_st,c.city_title",
                    "gl_ac a INNER JOIN geo_city c ON a.ac_city_id=c.city_id",
                    this.Text,
                    1,
                    "ID,Account Title,Level,LF #, City Title",
                    "10,20,4,8,12",
                    "T,T,T,T,T",
                    true,
                    "",
        ...
        {
            string lJoin = "INNER JOIN geo_city p ON kt.ac_city_id = p.city_id";
            try
            {
                // 0- 
                // 1- [FieldList],
                ...
                frmLookUp sForm = new frmLookUp(
                    "kt.ac_strid",
                    "kt.ac_title, kt.ac_atitle, kt.ac_st, p.city_title, kt.Ordering",
                    "gl_ac",
                    "GL COA",
                    1,
                    "ID,Account Title, Alternate Title,Ledger Folio,City,Ordering",
                    "10,20,20,10,10,10",
                    "T,T,T,T,T,T",
                    false,
                    lJoin
        ...
        private void btn_PinID_Click(object sender, EventArgs e)
        {
            if (btn_PinID.Text == "&Pin")
            {
                ...
                btn_PinID.Image = NizamiTrd.Properties.Resources.tiny_pinned;
                gbPin.Enabled = false;
            }
            else
            {
                btn_PinID.Text = "&Pin";
                btn_PinID.Image = NizamiTrd.Properties.Resources.tiny_pin;
                gbPin.Enabled = true;
            }
        }
        //
        ...
        private bool SaveData()
        {
            ...
            if (!FormValidation())
            {
                ...
                textAlert.Text = "Validation Error: Not Saved." + now.ToString("T");
                return false;
            }
            ...
            try
            {
                ...
                if (!fAlreadyExists)
                {
                    ...
                    string lStrNewVal = "";
                    lStrNewVal += "select max(" + fKeyFieldName + ") as maxvalue from " + fTableName;
                    lStrNewVal += " where ";
                    lStrNewVal += "  frm_id = " + fFormID.ToString();
                    // comment by bira
                    ...
                    tSQL += ", " + "N'" + StrF01.EnEpos(textaTitle.Text.ToString()) + "'";                   // 13- Alternate Title
                    //tSQL += ", " + "N'" + StringFun01.StrF01.EnEpos(utextaTitle.Text.ToString()) + "'";                   // 13- Alternate Title
                    ...
                    tSQL += ", " + cboCity.SelectedValue.ToString();                                          // 15a- city id
                ...
                else
                {
                    ...
                    tSQL += ", ac_atitle = " + "N'" + StrF01.EnEpos(textaTitle.Text.ToString()) + "'";
                    if (chkGroupTran.Checked == true)
                    {
                    ...
                    tSQL += ", ac_city_id = " + cboCity.SelectedValue.ToString();                                         // 15a- city id
                    ...
                    // D2Str tSQL += ", modified_date = '" +  StringFun01.StrF01.D2Str(DateTime.Now, true) + "'";
                    tSQL += ", modified_date = '" + StrF01.D2Str(DateTime.Now) + "'";
                    tSQL += " where ";
                    //tSQL += clsGVar.LGCY;
                    //tSQL += " and ";
                    tSQL += fKeyFieldName + " = " + fAcID.ToString();
                }
            ...
            if (clsDbManager.ExeOne(tSQL))
            {
                ...
                if (fAlreadyExists)
                {
                    textAlert.Text = "Existing ID: " + mtextID.Text.ToString() + " Modified ....";
                }
                else
                {
                    textAlert.Text = "New ID: " + mtextID.Text.ToString() + " Inserted ....";
                }
        ...
        private bool FormValidation()
        {
            ...
            if (textaTitle.Text.ToString() == "" || textaTitle.Text.ToString() == string.Empty)
            {
            ...
            //{
            //    ErrrMsg = StringFun01.StrF01.BuildErrMsg(ErrrMsg, "'" + "Alternate Account Title" + "' is Empty or Blank... " + "");
            //    rtnValue = false;
            //}
        ...
        private void btn_Address_Click(object sender, EventArgs e)
        {
          ...
          frmAddress_Master Dlg_Address = new frmAddress_Master(
              ...
              fAddressUID,
              "update " + fTableName + " set addr_uid = <<TOBEFillED>> where " + fKeyFieldName + " = " + fAcID
              );
          ...
          //
          // Update current form with new address uid
          string ltmpID = clsDbManager.GetTitleAc(
          "gl_ac",
          "ac_id",
          "addr_uid",
          fAcID
          ...
          //
          fAddressUID = clsDbManager.ConvInt64(ltmpID);
          lblAc_id.Text = fAcID + " / " + fAddressUID.ToString();
        ...
        private void lblPNaturalSide_Click(object sender, EventArgs e)
        {

        }

        private void mtextParentID_MaskInputRejected(object sender, MaskInputRejectedEventArgs e)
        {

        }

        private void btn_CityRefresh_Click( object sender, EventArgs e )
        {
          RefreshCityCbo();
        }

        private void tVCOA_DoubleClick( object sender, EventArgs e )
        {
          if (tVCOA.SelectedNode != null)
          {
            SelectAcIdFromTreeView("Auto");
          }
          else
          {
            DateTime lNow = DateTime.Now;
            textAlert.Text = "Double Click: Tree View Item not selected " + lNow.ToString("T");
          }

        }
        private void SelectAcIdFromTreeView( string pEditMode )
        {
          string ltext = tVCOA.SelectedNode.Text;
          bool lIsParent = true;
          if (pEditMode == "EditAsAccount")
          {
            lIsParent = false;
          }
          else
          {
            if (ltext.Length > (fGLMask.Length + 3 + 3))
            {
              string ltext1 = ltext.Substring(fGLMask.Length + 3 + 3, 3);
              if (ltext1 == "(T)")
              {
                lIsParent = false;
              }
          ...
          if (lIsParent)
          {
            if (!mtextParentID.Enabled)
            {
              MessageBox.Show("Parent Account ID is Protected, Please press Clear Button to enable !", lblFormTitle.Text.ToString());
              return;
            }
            mtextParentID.Text = StrF01.getIDFromTreeText(tVCOA, '[', ']');
            lblParentaTitle.Text = string.Empty;
            lblParentaTitle.Text = string.Empty;
            //lblpareSt.Text = string.Empty;
            lblParentOrdering.Text = string.Empty;
            lblParentLevel.Text = string.Empty;
            lblParentGroupTran.Text = string.Empty;
            lblParentAcType.Text = string.Empty;
            lblParentNaturalSide.Text = string.Empty;
            mtextParentID.Focus();
          }
          else
          {
            if (!mtextID.Enabled)
            {
              MessageBox.Show("Account ID is Protected, Please press Clear Button to enable !", lblFormTitle.Text.ToString());
              return;
            }
