...
namespace NizamiTrd
{
    ...
    public partial class frmItemOp : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.OPN;                         // Voucher/Doc Type ID
        ...
        private void LoadInitialControls()
        {
            ...
            clsDbManager.SetGridHeaderCmb(
                ...
                false,
                2);
            grdVoucher.Columns[(int)GColIO.WidDec].MinimumWidth = 20;
        ...
        private void SumVoc()
        {
            ...
            {
                // isBundle Check
                if (grdVoucher.Rows[i].Cells[(int)GColIO.isBundle].Value == null)
                {
                    str_IsBundle = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsBundle = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColIO.isBundle].Value.ToString());
                }

                // isMesh Check
                if (grdVoucher.Rows[i].Cells[(int)GColIO.isMesh].Value == null)
                {
                    str_IsMesh = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsMesh = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColIO.isMesh].Value.ToString());
                }

                if (str_IsBundle == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColIO.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Bundle].Value.ToString());
                }
                ...
                if (str_IsMesh == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColIO.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Bundle].Value.ToString())
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Width].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.WidDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Length].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.LenDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        ;
                ...
                if (str_IsBundle == "0" && str_IsMesh == "0")
                {
                    grdVoucher.Rows[i].Cells[(int)GColIO.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Qty].Value.ToString());
                ...
                if (grdVoucher.Rows[i].Cells[(int)GColIO.Amount].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColIO.Amount].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                        fTotalAmount += outValue;
                        fTotalQty += clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColIO.Qty].Value.ToString());
                    }
                }
        ...
        { 
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColIO.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GColIO.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    lSQL += " WHERE i.goodsitem_id=" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.ItemID].Value.ToString();
                    lSQL += "; SELECT goodsuom_st FROM gds_uom WHERE goodsuom_id=";
                    lSQL += grdVoucher.Rows[dGVRow].Cells[(int)GColIO.UOMID].Value.ToString();
                    ...
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColIO.isMesh].Value != null)
                    {
                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColIO.isBundle].Value.ToString()) == "0"
                            && clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColIO.isMesh].Value.ToString()) == "0")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Bundle].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Qty].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColIO.isBundle].Value.ToString()) == "1")
                        {
                            strItemDes += " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Bundle].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColIO.isMesh].Value.ToString()) == "1")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Bundle].Value.ToString()
                                + " x L=" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Length].Value.ToString()
                                + " x W=" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Width].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.MeshTotal].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Rate].Value.ToString();
                        }
                    ...
                    lSQL += ", " + 0;           // 9- Debit. 
                    lSQL += ", " + clsDbManager.ConvDecimal(grdVoucher.Rows[dGVRow].Cells[(int)GColIO.Amount].Value.ToString());           // 10- Credit
                    lSQL += ", " + 0;          // 11- Combo 1 
                    lSQL += ", 0"; //is Checked
                    lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GColIO.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GColIO.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
