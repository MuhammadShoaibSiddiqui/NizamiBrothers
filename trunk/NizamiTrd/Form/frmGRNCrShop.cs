...
namespace NizamiTrd
{
    ...
    public partial class frmGRNCrShop : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.GRN;                         // Voucher/Doc Type ID
        ...
        private void LoadInitialControls()
        {
            ...
            clsDbManager.SetGridHeaderCmb(
                ...
                false,
                2);
            grdVoucher.Columns[(int)GColGrnS.WidDec].MinimumWidth = 20;
...
//order by a.ac_title
        ...
        private void SumVoc()
        {
            ...
            {
                // isBundle Check
                if (grdVoucher.Rows[i].Cells[(int)GColGrnS.isBundle].Value == null)
                {
                    str_IsBundle = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsBundle = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColGrnS.isBundle].Value.ToString());
                }

                // isMesh Check
                if (grdVoucher.Rows[i].Cells[(int)GColGrnS.isMesh].Value == null)
                {
                    str_IsMesh = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsMesh = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColGrnS.isMesh].Value.ToString());
                }

                if (str_IsBundle == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColGrnS.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Bundle].Value.ToString());
                }
                ...
                if (str_IsMesh == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColGrnS.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Bundle].Value.ToString())
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Width].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.WidDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Length].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.LenDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        ;
                ...
                if (str_IsBundle == "0" && str_IsMesh == "0")
                {
                    grdVoucher.Rows[i].Cells[(int)GColGrnS.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Qty].Value.ToString());
                ...
                if (grdVoucher.Rows[i].Cells[(int)GColGrnS.Amount].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColGrnS.Amount].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                        fTotalAmount += outValue;
                        fTotalQty += clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnS.Qty].Value.ToString());
                    }
                }
        ...
        { 
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    lSQL = "SELECT gg.Group_st + i.goodsitem_st AS Item_ST ";
                    lSQL += " FROM gds_item i INNER JOIN gds_Group gg ON i.Group_id=gg.Group_id ";
                    lSQL += " WHERE i.goodsitem_id=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.ItemID].Value.ToString();
                    lSQL += "; SELECT goodsuom_st FROM gds_uom WHERE goodsuom_id=";
                    lSQL += grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.UOMID].Value.ToString();
                    ...
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.isMesh].Value != null)
                    {
                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.isBundle].Value.ToString()) == "0"
                            && clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.isMesh].Value.ToString()) == "0")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Bundle].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Qty].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.isBundle].Value.ToString()) == "1")
                        {
                            strItemDes += " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Bundle].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.isMesh].Value.ToString()) == "1")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Bundle].Value.ToString()
                                + " x L=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Length].Value.ToString()
                                + " x W=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Width].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.MeshTotal].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Rate].Value.ToString();
                        }
                    ...
                    lSQL += ", '" + strItemDes + "'";      // 8- Narration 
                    lSQL += ", " + 0;           // 9- Debit. 
                    lSQL += ", " + clsDbManager.ConvDecimal(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnS.Amount].Value.ToString());           // 10- Credit
                    lSQL += ", " + 0;          // 11- Combo 1 
                    lSQL += ", 0"; //is Checked
                    lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GColGrnS.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GColGrnS.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
