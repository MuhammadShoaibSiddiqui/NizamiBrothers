...
namespace NizamiTrd
{
    ...
    public partial class frmCRVoc : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.CRV;                         // Voucher/Doc Type ID
        ...
        private void frmJournalVoc_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Space)
            {
                ...
                if (grdVoucher.Rows.Count > 0)
                {
                    ...
                    //lblAcID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColCR.acid].Value.ToString();
                    lblAccountName.Tag = grdVoucher.Rows[fEditRow].Cells[(int)GColCR.acid].Value.ToString();
                    msk_AccountID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColCR.acstrid].Value.ToString();
                    lblAccountName.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColCR.actitle].Value.ToString();
                    txtNarration.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColCR.Desc].Value.ToString();
                    txtDebit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColCR.debit].Value.ToString();
                    txtCredit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColCR.credit].Value.ToString();
                    btn_Add.Text = "&Update";
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            {
                //grdVoucher.Rows[fEditRow].Cells[(int)GColCR.acid].Value = lblAcID.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColCR.acid].Value = lblAccountName.Tag;
                grdVoucher.Rows[fEditRow].Cells[(int)GColCR.acstrid].Value = msk_AccountID.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColCR.actitle].Value = lblAccountName.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColCR.Desc].Value = txtNarration.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColCR.debit].Value = txtDebit.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColCR.credit].Value = txtCredit.Text;
                btn_Add.Text = "&Add";
                pnlVocTran.Visible = false;
                grdVoucher.Focus();
                //msk_AccountID.Focus();
        ...
        private void SumVoc()
        {
            ...
            {
                if (grdVoucher.Rows[i].Cells[(int)GColCR.debit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColCR.debit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                ...
                if (grdVoucher.Rows[i].Cells[(int)GColCR.credit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColCR.credit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
        ...
        private bool GridValid()
        {
            ...
            try
            {
                ...
                {
                    // Check GL Account ID
                    if (grdVoucher.Rows[lLastRow].Cells[(int)GColCR.acstrid].Value == null) // || 
                    {
                        rtnValue = false;
                    }
                    else
                    {
                        if ((grdVoucher.Rows[lLastRow].Cells[(int)GColCR.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            rtnValue = false;
                        }
                    }
        ...
        private bool PrepareDocMaster()
        {
            ...
            try
            {
                ...
                //
                lSQL += ", " + lblVocCodeName.Tag; //grdVoucher.Rows[dGVRow].Cells[(int)GColCR.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                //                                                                                       // 5- Ac Title NA
                lSQL += ", '" + "Cash Receipt Voucher" + "'";      // 8- Narration 
                lSQL += ", " +  float.Parse(lblTotalCr.Text);           // 9- Debit. 
                lSQL += ", " + 0;          // 10- Credit
                lSQL += ", " + 0;          // 11- Combo 1 
                lSQL += ", 0"; //is Checked
                lSQL += ")";
        ...
        private bool PrepareDocDetail()
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColCR.acstrid].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GColCR.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    lSQL += ", " + fDocID.ToString();                                                        // 2- Form 1- Voucher_id
                    //
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColCR.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                    //                                                                                       // 5- Ac Title NA
                    lSQL += ", '" + StrF01.EnEpos(grdVoucher.Rows[dGVRow].Cells[(int)GColCR.Desc].Value.ToString()) + "'";      // 8- Narration 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColCR.debit].Value.ToString();           // 9- Debit. 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColCR.credit].Value.ToString();          // 10- Debit. 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColCR.snum].Value.ToString();          // 11- Combo 1 
                    lSQL += ", 0"; //is Checked
                    lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GColCR.acstrid].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GColCR.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
                    {
                        ...
                        grdVoucher.Rows.Add(
                            ( dRow.ItemArray.GetValue((int)GColCR.Status) == DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColCR.Status).ToString()),                       // 0-
                            ( dRow.ItemArray.GetValue((int)GColCR.tranid) == DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColCR.tranid).ToString()),                           // 1-
                            (dRow.ItemArray.GetValue((int)GColCR.snum)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColCR.snum).ToString() ),                           // 1-
                            (dRow.ItemArray.GetValue((int)GColCR.acid)==DBNull.Value ? "0" : dRow.ItemArray.GetValue((int)GColCR.acid).ToString() ),                             // 3-
                            (dRow.ItemArray.GetValue((int)GColCR.acstrid)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColCR.acstrid).ToString() ),                          // 4-
                            (dRow.ItemArray.GetValue((int)GColCR.actitle)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColCR.actitle).ToString() ),                          // 5-
                            (dRow.ItemArray.GetValue((int)GColCR.Desc)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColCR.Desc).ToString() ),                            // 6-
                            (dRow.ItemArray.GetValue((int)GColCR.debit)==DBNull.Value ? "0" : dRow.ItemArray.GetValue((int)GColCR.debit).ToString() ),                            // 9-
                            (dRow.ItemArray.GetValue((int)GColCR.credit)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColCR.credit).ToString() )                           // 10-
                            );
