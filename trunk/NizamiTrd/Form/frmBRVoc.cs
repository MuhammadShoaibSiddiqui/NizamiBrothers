...
namespace NizamiTrd
{
    ...
    public partial class frmBRVoc : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.BRV;                         // Voucher/Doc Type ID
        ...
        private void frmJournalVoc_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Space)
            {
                ...
                if (grdVoucher.Rows.Count > 0)
                {
                    ...
                    lblAccountName.Tag = grdVoucher.Rows[fEditRow].Cells[(int)GColBR.acid].Value.ToString();
                    msk_AccountID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColBR.acstrid].Value.ToString();
                    lblAccountName.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColBR.actitle].Value.ToString();
                    txtNarration.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColBR.Desc].Value.ToString();
                    txtDebit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColBR.debit].Value.ToString();
                    txtCredit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColBR.credit].Value.ToString();
                    btn_Add.Text = "&Update";
        ...
        private void msk_AccountID_Leave(object sender, EventArgs e)
        {
            ...
            {
                //if (msk_AccountID.Text. == SendKeys.Send("{TAB}"))
                //{
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            else if (btn_Add.Text.ToString() == "&Update")
            {
                ...
                grdVoucher.Rows[fEditRow].Cells[(int)GColBR.acid].Value = lblAccountName.Tag;
                grdVoucher.Rows[fEditRow].Cells[(int)GColBR.acstrid].Value = msk_AccountID.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColBR.actitle].Value = lblAccountName.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColBR.Desc].Value = txtNarration.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColBR.debit].Value = txtDebit.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColBR.credit].Value = txtCredit.Text;
                btn_Add.Text = "&Add";
                pnlVocTran.Visible = false;
                grdVoucher.Focus();
                //msk_AccountID.Focus();
        ...
        private void SumVoc()
        {
            ...
            {
                if (grdVoucher.Rows[i].Cells[(int)GColBR.debit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColBR.debit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                ...
                if (grdVoucher.Rows[i].Cells[(int)GColBR.credit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColBR.credit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
        ...
        private bool GridValid()
        {
            ...
            try
            {
                ...
                {
                    // Check GL Account ID
                    if (grdVoucher.Rows[lLastRow].Cells[(int)GColBR.acstrid].Value == null) // || 
                    {
                        rtnValue = false;
                    }
                    else
                    {
                        if ((grdVoucher.Rows[lLastRow].Cells[(int)GColBR.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            rtnValue = false;
                        }
                    }
        ...
        private bool PrepareDocMaster()
        {
            bool rtnValue = true;
            string lSQL = string.Empty;
            fDocAmt = clsDbManager.ConvDecimal(lblTotalCr.Text);
        ...
        private bool PrepareDocDetail()
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColBR.acstrid].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GColBR.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    lSQL += ", " + fDocID.ToString();                                                        // 2- Form 1- Voucher_id
                    //
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColBR.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                    //                                                                                       // 5- Ac Title NA
                    lSQL += ", '" + StrF01.EnEpos(grdVoucher.Rows[dGVRow].Cells[(int)GColBR.Desc].Value.ToString()) + "'";      // 8- Narration 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColBR.debit].Value.ToString();           // 9- Debit. 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColBR.credit].Value.ToString();          // 10- Debit. 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColBR.snum].Value.ToString();          // 11- Combo 1 
                    lSQL += ", 0"; //is Checked
                    lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GColBR.acstrid].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GColBR.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
                    {
                        ...
                        grdVoucher.Rows.Add(
                            ( dRow.ItemArray.GetValue((int)GColBR.Status) == DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColBR.Status).ToString()),                       // 0-
                            ( dRow.ItemArray.GetValue((int)GColBR.tranid) == DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColBR.tranid).ToString()),                           // 1-
                            (dRow.ItemArray.GetValue((int)GColBR.snum)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColBR.snum).ToString() ),                           // 1-
                            (dRow.ItemArray.GetValue((int)GColBR.acid)==DBNull.Value ? "0" : dRow.ItemArray.GetValue((int)GColBR.acid).ToString() ),                             // 3-
                            (dRow.ItemArray.GetValue((int)GColBR.acstrid)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColBR.acstrid).ToString() ),                          // 4-
                            (dRow.ItemArray.GetValue((int)GColBR.actitle)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColBR.actitle).ToString() ),                          // 5-
                            (dRow.ItemArray.GetValue((int)GColBR.Desc)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColBR.Desc).ToString() ),                            // 6-
                            (dRow.ItemArray.GetValue((int)GColBR.debit)==DBNull.Value ? "0" : dRow.ItemArray.GetValue((int)GColBR.debit).ToString() ),                            // 9-
                            (dRow.ItemArray.GetValue((int)GColBR.credit)==DBNull.Value ? "" : dRow.ItemArray.GetValue((int)GColBR.credit).ToString() )                           // 10-
                            );
