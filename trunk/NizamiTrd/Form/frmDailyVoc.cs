...
namespace NizamiTrd
{
    ...
    public partial class frmDailyVoc : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.DVE;                         // Voucher/Doc Type ID
        ...
        private void frmJournalVoc_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Space)
            {
                ...
                if (grdVoucher.Rows.Count > 0)
                {
                    ...
                    //lblAcID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColD.acid].Value.ToString();
                    //msk_AccountID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColD.acstrid].Value.ToString();
                    //lblAccountName.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColD.actitle].Value.ToString();
                    txtNarration.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColD.Desc].Value.ToString();
                    txtDebit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColD.debit].Value.ToString();
                    //txtCredit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColD.credit].Value.ToString();
                    btn_Add.Text = "&Update";
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            {
                //grdVoucher.Rows[fEditRow].Cells[(int)GColD.acid].Value = lblAcID.Text;
                //grdVoucher.Rows[fEditRow].Cells[(int)GColD.acstrid].Value = msk_AccountID.Text;
                //grdVoucher.Rows[fEditRow].Cells[(int)GColD.actitle].Value = lblAccountName.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColD.Desc].Value = txtNarration.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColD.debit].Value = txtDebit.Text;
                //grdVoucher.Rows[fEditRow].Cells[(int)GColD.credit].Value = txtCredit.Text;
                btn_Add.Text = "&Add";
                pnlVocTran.Visible = false;
                grdVoucher.Focus();
                //txtNarration.Focus();
        ...
        private void SumVoc()
        {
            ...
            {
                if (grdVoucher.Rows[i].Cells[(int)GColD.debit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColD.debit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                ...
                //if (grdVoucher.Rows[i].Cells[(int)GColD.credit].Value != null)
                //{
                //    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColD.credit].Value.ToString(), out outValue);
                //    if (bcheck)
                //    {
                //        rtnVal += outValue;
        ...
        private bool GridValid()
        {
            ...
            try
            {
                ...
                {
                    // Check GL Account ID
                    //if (grdVoucher.Rows[lLastRow].Cells[(int)GColD.acstrid].Value == null) // || 
                    //{
                    //    rtnValue = false;
                    //}
                    //else
                    //{
                    //    if ((grdVoucher.Rows[lLastRow].Cells[(int)GColD.acstrid].Value.ToString()).Trim(' ', '-') == "")
                    //    {
                    //        rtnValue = false;
                    //    }
                    //}
        ...
        private bool PrepareDocMaster()
        {
            ...
            try
            {
                ...
                if (lblDocID.Text.ToString().Trim(' ', '-') == "")
                {
                    ...
                    fDocID = clsDbManager.GetNextValDocID("dt_tran", "doc_id", fDocWhere, "");
                    //
                    lSQL = "insert into dt_tran (";
                    lSQL += "  doc_vt_id ";                                         // 1-
                ...
                {
                    fDocWhere = " Doc_vt_id = " + fDocTypeID.ToString(); //jvr=267
                    fDocWhere += " AND doc_Fiscal_ID = " + fDocFiscal.ToString();
                    fDocWhere += " AND Doc_ID = " + String.Format("{0,0}",lblDocID.Text.ToString());
                    //if (clsDbManager.IDAlreadyExistWw("dt_tran", "doc_id", DocWhere("")))
                    if (clsDbManager.IDAlreadyExistWw("dt_tran", "doc_id", fDocWhere)) 
                    {
                        fDocAlreadyExists = true;
                        lSQL = "delete from dt_trandtl ";
        ...
        private bool PrepareDocDetail()
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    //
                    //if (grdVoucher.Rows[dGVRow].Cells[(int)GColD.acstrid].Value == null)
                    //{
                    //    if (dGVRow == fLastRow)
                    //    {
                    //        continue;
                    ...
                    //else
                    //{
                    //    if ((grdVoucher.Rows[dGVRow].Cells[(int)GColD.acstrid].Value.ToString()).Trim(' ', '-') == "")
                    //    {
                    //        //lBlank = true;
                    //        if (dGVRow == fLastRow)
                    //        {
                    ...
                    //
                    //lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColD.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                    //                                                                                       // 5- Ac Title NA
                    lSQL += ", '" + StrF01.EnEpos(grdVoucher.Rows[dGVRow].Cells[(int)GColD.Desc].Value.ToString()) + "'";      // 8- Narration 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColD.debit].Value.ToString();           // 9- Debit. 
                    //lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColD.credit].Value.ToString();          // 10- Debit. 
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    //if (pdGv.Rows[dGVRow].Cells[(int)GColD.acstrid].Value == null)
                    //{
                    //    if (dGVRow == fLastRow)
                    //    {
                    //        break;
                    ...
                    //else
                    //{
                    //    if ((pdGv.Rows[dGVRow].Cells[(int)GColD.acstrid].Value.ToString()).Trim(' ', '-') == "")
                    //    {
                    //        //lBlank = true;
                    //        if (dGVRow == fLastRow)
                    //        {
...
//where doc_vt_id=1 and doc_fiscal_id=1
        ...
        private void PopulateRecords()
        {
            ...
            //d.AC_ID, a.ac_strid, a.ac_title, d.CREDIT, 
            tSQL += " select 1 as Status, d.Serial_No, d.Serial_Order, ";
            tSQL += " d.NARRATION, d.DEBIT, t.Doc_Remarks";
            tSQL += " from dt_Tran t Left Outer join dt_trandtl d on t.doc_vt_id=d.doc_vt_id";
            ...
            try
            {
                ds = clsDbManager.GetData_Set(tSQL, "dt_tran");
                if (ds.Tables[0].Rows.Count > 0)
                {
                    //fAlreadyExists = true;
                    dRow = ds.Tables[0].Rows[0];
                    ...
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
                    {
                        ...
                        grdVoucher.Rows.Add(
                            dRow.ItemArray.GetValue((int)GColD.Status).ToString(),                       // 0-
                            dRow.ItemArray.GetValue((int)GColD.tranid).ToString(),                           // 1-
                            dRow.ItemArray.GetValue((int)GColD.snum).ToString(),                           // 1-
                            //dRow.ItemArray.GetValue((int)GColD.acid).ToString(),                             // 3-
                            //dRow.ItemArray.GetValue((int)GColD.acstrid).ToString(),                          // 4-
                            //dRow.ItemArray.GetValue((int)GColD.actitle).ToString(),                          // 5-
                            dRow.ItemArray.GetValue((int)GColD.Desc).ToString(),                            // 6-
                            dRow.ItemArray.GetValue((int)GColD.debit).ToString()                            // 9-
                            //dRow.ItemArray.GetValue((int)GColD.credit).ToString()                           // 10-
                            );
        ...
        //Populate Previous Recordset 
        private void PopulatePrevRecords()
        {
            DataSet ds = new DataSet();
            DataRow dRow;
            string tSQL = string.Empty;
            ...
            tSQL  = " select 1 as Status, d.Serial_No, d.Serial_Order,  d.NARRATION, d.DEBIT, t.Doc_Remarks ";
            tSQL += " from dt_Tran t Left Outer join dt_trandtl d on t.doc_vt_id=d.doc_vt_id ";
            tSQL += " and t.doc_fiscal_id=d.doc_fiscal_id  and t.Doc_ID=d.Doc_ID ";
            tSQL += " where  t.doc_vt_id=" + fDocTypeID.ToString() + " and t.doc_fiscal_id=1 and t.doc_status=1 ";
	        tSQL += " and t.doc_id IN (SELECT TOP 1 doc_id FROM dt_tran WHERE doc_date<'" + StrF01.D2Str(msk_VocDate) + "'";
            tSQL += " and doc_vt_id=" + fDocTypeID.ToString() + " and doc_fiscal_id=1 and doc_status=1 ORDER BY doc_id DESC) ";
            tSQL += " order by d.SERIAL_No";
        ...
        private void btn_View_Click(object sender, EventArgs e)
        {
            ...
            CrDailyVoc rpt1 = new CrDailyVoc();
