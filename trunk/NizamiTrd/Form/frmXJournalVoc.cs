...
namespace NizamiTrd
{
    ...
    public partial class frmXJournalVoc : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.XCV;                         // Voucher/Doc Type ID
        ...
        private void frmJournalVoc_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Space)
            {
                ...
                if (grdVoucher.Rows.Count > 0)
                {
                    ...
                    lblAcID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColX.acid].Value.ToString();
                    msk_AccountID.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColX.acstrid].Value.ToString();
                    lblAccountName.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColX.actitle].Value.ToString();
                    txtNarration.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColX.Desc].Value.ToString();
                    txtDebit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColX.debit].Value.ToString();
                    txtCredit.Text = grdVoucher.Rows[fEditRow].Cells[(int)GColX.credit].Value.ToString();
                    btn_Add.Text = "&Update";
        ...
        private void btn_Add_Click(object sender, EventArgs e)
        {
            ...
            {
                grdVoucher.Rows[fEditRow].Cells[(int)GColX.acid].Value = lblAcID.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColX.acstrid].Value = msk_AccountID.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColX.actitle].Value = lblAccountName.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColX.Desc].Value = txtNarration.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColX.debit].Value = txtDebit.Text;
                grdVoucher.Rows[fEditRow].Cells[(int)GColX.credit].Value = txtCredit.Text;
                btn_Add.Text = "&Add";
                pnlVocTran.Visible = false;
                grdVoucher.Focus();
                //msk_AccountID.Focus();
        ...
        private void SumVoc()
        {
            ...
            {
                if (grdVoucher.Rows[i].Cells[(int)GColX.debit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColX.debit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                ...
                if (grdVoucher.Rows[i].Cells[(int)GColX.credit].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColX.credit].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
        ...
        private bool GridValid()
        {
            ...
            try
            {
                ...
                {
                    // Check GL Account ID
                    if (grdVoucher.Rows[lLastRow].Cells[(int)GColX.acstrid].Value == null) // || 
                    {
                        rtnValue = false;
                    }
                    else
                    {
                        if ((grdVoucher.Rows[lLastRow].Cells[(int)GColX.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            rtnValue = false;
                        }
                    }
        ...
        private bool PrepareDocDetail()
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColX.acstrid].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GColX.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    //
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColX.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                    //                                                                                       // 5- Ac Title NA
                    lSQL += ", '" + StrF01.EnEpos(grdVoucher.Rows[dGVRow].Cells[(int)GColX.Desc].Value.ToString()) + "'";      // 8- Narration 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColX.debit].Value.ToString();           // 9- Debit. 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColX.credit].Value.ToString();          // 10- Debit. 
                    lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColX.snum].Value.ToString();          // 11- Combo 1 
                    lSQL += ", 0"; //is Checked
                    lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GColX.acstrid].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GColX.acstrid].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
...
//where doc_vt_id=1 and doc_fiscal_id=1
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
                    {
                        ...
                        grdVoucher.Rows.Add(
                            dRow.ItemArray.GetValue((int)GColX.Status).ToString(),                       // 0-
                            dRow.ItemArray.GetValue((int)GColX.tranid).ToString(),                           // 1-
                            dRow.ItemArray.GetValue((int)GColX.snum).ToString(),                           // 1-
                            dRow.ItemArray.GetValue((int)GColX.acid).ToString(),                             // 3-
                            dRow.ItemArray.GetValue((int)GColX.acstrid).ToString(),                          // 4-
                            dRow.ItemArray.GetValue((int)GColX.actitle).ToString(),                          // 5-
                            dRow.ItemArray.GetValue((int)GColX.Desc).ToString(),                            // 6-
                            dRow.ItemArray.GetValue((int)GColX.debit).ToString(),                            // 9-
                            dRow.ItemArray.GetValue((int)GColX.credit).ToString()                           // 10-
                            );
        ...
        private void chk_Edit_Click(object sender, EventArgs e)
        {
            if (chk_Edit.Checked == true && fEditMod==true)
            {
                txtManualDoc.Enabled = true;
                msk_VocDate.Enabled = true;
            }
        ...
        private void btn_PinIDDr_Click(object sender, EventArgs e)
        {
            if (btn_PinIDDr.Text == "&Pin")
            {
                btn_PinIDDr.Text = "&Un Pin";
                btn_PinIDDr.Image = Properties.Resources.tiny_pinned;
                txtDebit.Enabled = false;
                btn_PinIDCr.Text = "&Pin";
                txtCredit.Enabled = true;
                txtNarration.Text = "RECEIPT ";
            }
            else
            {
                btn_PinIDDr.Text = "&Pin";
                btn_PinIDDr.Image = Properties.Resources.tiny_pin;
                txtDebit.Enabled = true;
                btn_PinIDCr.Text = "&Un Pin";
                txtCredit.Enabled = false;
                txtNarration.Text = "RECEIPT ";
            }
        }

        private void btn_PinIDCr_Click(object sender, EventArgs e)
        {
            if (btn_PinIDCr.Text == "&Pin")
            {
                btn_PinIDCr.Text = "&Un Pin";
                btn_PinIDCr.Image = Properties.Resources.tiny_pinned;
                txtCredit.Enabled = false;
                btn_PinIDDr.Text = "&Pin";
                txtDebit.Enabled = true;
                txtNarration.Text = "PAYMENT ";
            ...
            {
                btn_PinIDCr.Text = "&Pin";
                btn_PinIDCr.Image = Properties.Resources.tiny_pin;
                txtCredit.Enabled = true;
                btn_PinIDDr.Text = "&Un Pin";
                txtDebit.Enabled = false;
                txtNarration.Text = "PAYMENT ";
