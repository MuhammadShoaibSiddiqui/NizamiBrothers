...
namespace NizamiTrd
{
    ...
    public partial class frmContractNew : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.CNT;                         // Voucher/Doc Type ID
        ...
        private void LoadInitialControls()
        {
            ...
            clsDbManager.SetGridHeaderCmb(
                ...
                " 5,10, 3, 5, 2, 5, 7, 7, 7, 7, 7, 4, 4, 7, 7, 7,20",
                " T, T, H, T, H, H,N2,N2,N2,N2,N2,CH,CH,N2,N2,N2,N2",
                ...
                false,
                2);
            grdVoucher.Columns[(int)GColCont.WidDec].MinimumWidth = 20;
        ...
        private void SumVoc()
        {
            ...
            {
                // isBundle Check
                if (grdVoucher.Rows[i].Cells[(int)GColCont.isBundle].Value == null)
                {
                    str_IsBundle = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsBundle = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColCont.isBundle].Value.ToString());
                }

                // isMesh Check
                if (grdVoucher.Rows[i].Cells[(int)GColCont.isMesh].Value == null)
                {
                    str_IsMesh = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsMesh = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColCont.isMesh].Value.ToString());
                }

                if (str_IsBundle == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColCont.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Bundle].Value.ToString());
                }
                ...
                if (str_IsMesh == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColCont.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Bundle].Value.ToString())
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Width].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.WidDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Length].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.LenDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        ;
                ...
                if (str_IsBundle == "0" && str_IsMesh == "0")
                {
                    grdVoucher.Rows[i].Cells[(int)GColCont.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Qty].Value.ToString());
                ...
                if (grdVoucher.Rows[i].Cells[(int)GColCont.Amount].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColCont.Amount].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                        fTotalAmount += outValue;
                        fTotalQty += clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCont.Qty].Value.ToString());
                    }
                }
        ...
        { 
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColCont.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GColCont.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    lSQL += " WHERE i.goodsitem_id=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.ItemID].Value.ToString();
                    lSQL += "; SELECT goodsuom_st FROM gds_uom WHERE goodsuom_id=";
                    lSQL += grdVoucher.Rows[dGVRow].Cells[(int)GColCont.UOMID].Value.ToString();
                    ...
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColCont.isMesh].Value != null)
                    {
                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColCont.isBundle].Value.ToString()) == "0"
                            && clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColCont.isMesh].Value.ToString()) == "0")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Bundle].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Qty].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColCont.isBundle].Value.ToString()) == "1")
                        {
                            strItemDes += " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Bundle].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColCont.isMesh].Value.ToString()) == "1")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Bundle].Value.ToString()
                                + " x L=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Length].Value.ToString()
                                + " x W=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Width].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.MeshTotal].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Rate].Value.ToString();
                        }
                    }
                    ...
                    //lSQL += ", " + clsDbManager.ConvDecimal(grdVoucher.Rows[dGVRow].Cells[(int)GColCont.Amount].Value.ToString());           // 9- Debit. 
                    //lSQL += ", " + 0;           // 10- Credit
                    //lSQL += ", " + 0;          // 11- Combo 1 
                    //lSQL += ", 0"; //is Checked
                    //lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GColCont.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GColCont.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
        ...
        private void LookUp_Voc()
        {
            ...
            frmLookUp sForm = new frmLookUp(
                    "doc_id",
                    "doc_strid, doc_date, CASE WHEN doc_status=1 THEN 'Complete' ELSE 'Running' END AS Doc_Status, doc_amt",
                    "ct_Tran",
                    this.Text.ToString(),
                    1,
                    "Doc ID,Voucher ID,Date,Status,Amount",
                    "6,8,8,10,12",
                    " T, T, T, T,N2",
                    true,
                    "",
        ...
        private void PopulateRecords()
        {
            ...
            tSQL += " it.doc_amt, it.GLID, ga.ac_title, ga.ac_strid, it.Doc_Status ";
