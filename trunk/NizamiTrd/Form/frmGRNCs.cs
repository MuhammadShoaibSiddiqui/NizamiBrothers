...
namespace NizamiTrd
{
    ...
    public partial class frmGRNCs : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.CGR;                         // Voucher/Doc Type ID
        ...
        private void LoadInitialControls()
        {
            ...
            clsDbManager.SetGridHeaderCmb(
                ...
                false,
                2);
            grdVoucher.Columns[(int)GColGrnCs.WidDec].MinimumWidth = 20;
        ...
        private void SumVoc()
        {
            ...
            {
                // isBundle Check
                if (grdVoucher.Rows[i].Cells[(int)GColGrnCs.isBundle].Value == null)
                {
                    str_IsBundle = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsBundle = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColGrnCs.isBundle].Value.ToString());
                }

                // isMesh Check
                if (grdVoucher.Rows[i].Cells[(int)GColGrnCs.isMesh].Value == null)
                {
                    str_IsMesh = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsMesh = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColGrnCs.isMesh].Value.ToString());
                }

                if (str_IsBundle == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColGrnCs.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Bundle].Value.ToString());
                }
                ...
                if (str_IsMesh == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColGrnCs.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Bundle].Value.ToString())
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Width].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.WidDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Length].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.LenDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        ;
                ...
                if (str_IsBundle == "0" && str_IsMesh == "0")
                {
                    grdVoucher.Rows[i].Cells[(int)GColGrnCs.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Qty].Value.ToString());
                ...
                if (grdVoucher.Rows[i].Cells[(int)GColGrnCs.Amount].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Amount].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                        fTotalAmount += outValue;
                        fTotalQty += clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColGrnCs.Qty].Value.ToString());
                    }
                }
        ...
        private bool PrepareDocMaster()
        {
            ...
            string str_CashPurcCode = string.Empty;

            try
            {
                string lDocDateStr = StrF01.D2Str(msk_VocDate);
                DateTime lDocDate = DateTime.Parse(lDocDateStr);
                str_CashPurcCode = clsDbManager.GetConfigCode("cmn_Config", "CashPurcID", "", "");
                ...
                lSQL += ", " + fDocID.ToString();                          // 2- Form 1- Voucher_id
                //
                lSQL += ", " + clsDbManager.ConvInt(str_CashPurcCode); //msk_AccountID.Tag; //grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                //                                                                                       // 5- Ac Title NA
        ...
        private bool PrepareDocDetail()
        {
            ...
            //            if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.isMesh].Value.ToString()) == "0")
            //            {
            //                strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Bundle].Value.ToString()
            //                    + " L=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Length].Value.ToString()
            //                    + " W=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Width].Value.ToString()
            //                    + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Qty].Value.ToString()
            //                    + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Rate].Value.ToString();
            //            }
            ...
            //        lSQL += ", " + fDocID.ToString();                                                        // 2- Form 1- Voucher_id
            //        //
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.ItemID].Value.ToString();            // 2- Serial Order replaced with SNo. 
            //        //                                                                                       // 5- Ac Title NA
            //        lSQL += ", '" + StrF01.EnEpos(grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.ItemName].Value.ToString()) + "'";      // 8- Narration 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.UOMID].Value.ToString();            // 2- Serial Order replaced with SNo. 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.GodownID].Value.ToString();            // 2- Serial Order replaced with SNo. 

            //        lSQL += ", 0"; //+ grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Qty].Value.ToString();           // 9- Debit. 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Qty].Value.ToString();           // 9- Debit. 

            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Rate].Value.ToString();          // 10- Debit. 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Bundle].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.MeshTotal].Value.ToString();          // 11- Combo 1 

            //        lSQL += ", " + clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.isBundle]);           // 11- Combo 1 
            //        lSQL += ", " + clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.isMesh]);           // 11- Combo 1 

            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Length].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.LenDec].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.Width].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColGrn.WidDec].Value.ToString();          // 11- Combo 1 
        ...
        { 
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    lSQL = "SELECT gg.Group_st + i.goodsitem_st AS Item_ST ";
                    lSQL += " FROM gds_item i INNER JOIN gds_Group gg ON i.Group_id=gg.Group_id ";
                    lSQL += " WHERE i.goodsitem_id=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.ItemID].Value.ToString();
                    lSQL += "; SELECT goodsuom_st FROM gds_uom WHERE goodsuom_id=";
                    lSQL += grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.UOMID].Value.ToString();
                    ...
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.isMesh].Value != null)
                    {
                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.isBundle].Value.ToString()) == "0"
                            && clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.isMesh].Value.ToString()) == "0")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Bundle].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Qty].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.isBundle].Value.ToString()) == "1")
                        {
                            strItemDes += " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Bundle].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.isMesh].Value.ToString()) == "1")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Bundle].Value.ToString()
                                + " x L=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Length].Value.ToString()
                                + " x W=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Width].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.MeshTotal].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Rate].Value.ToString();
                        }
                    ...
                    lSQL += ", '" + strItemDes + "'";      // 8- Narration 
                    lSQL += ", " + 0;           // 9- Debit. 
                    lSQL += ", " + clsDbManager.ConvDecimal(grdVoucher.Rows[dGVRow].Cells[(int)GColGrnCs.Amount].Value.ToString());           // 10- Credit
                    lSQL += ", " + 0;          // 11- Combo 1 
                    lSQL += ", 0"; //is Checked
                    lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GColGrnCs.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GColGrnCs.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
        ...
        private void btn_PinIDAc_Click(object sender, EventArgs e)
        {
            if (btn_PinIDAc.Text == "&Pin")
            {
                ...
                btn_PinIDAc.Image = NizamiTrd.Properties.Resources.tiny_pinned;
                msk_AccountID.Enabled = false;
            }
            else
            {
                btn_PinIDAc.Text = "&Pin";
                btn_PinIDAc.Image = NizamiTrd.Properties.Resources.tiny_pin;
                msk_AccountID.Enabled = true;
            }
        }

        private void btn_PinID_Click_1(object sender, EventArgs e)
        {
