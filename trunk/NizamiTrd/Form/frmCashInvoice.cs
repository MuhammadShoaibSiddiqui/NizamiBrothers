...
namespace NizamiTrd
{
    ...
    public partial class frmCashInvoice : Form
    {
        ...
        //
        int fDocTypeID = clsGVar.CIV;                         // Voucher/Doc Type ID
        ...
        private void LoadInitialControls()
        {
            ...
            clsDbManager.SetGridHeaderCmb(
                ...
                false,
                2);
            grdVoucher.Columns[(int)GColCsInv.WidDec].MinimumWidth = 20;
        ...
        private void ButtonImageSetting()
        {
            ...
            btn_PinIDAc.Image = Properties.Resources.tiny_pin;
        ...
        private void SumVoc()
        {
            ...
            {
                // isBundle Check
                if (grdVoucher.Rows[i].Cells[(int)GColCsInv.isBundle].Value == null)
                {
                    str_IsBundle = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsBundle = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColCsInv.isBundle].Value.ToString());
                }

                // isMesh Check
                if (grdVoucher.Rows[i].Cells[(int)GColCsInv.isMesh].Value == null)
                {
                    str_IsMesh = clsDbManager.ConvBit("False");
                }
                else
                {
                    str_IsMesh = clsDbManager.ConvBit(grdVoucher.Rows[i].Cells[(int)GColCsInv.isMesh].Value.ToString());
                }

                if (str_IsBundle == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColCsInv.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Bundle].Value.ToString());
                }
                ...
                if (str_IsMesh == "1")
                {
                    grdVoucher.Rows[i].Cells[(int)GColCsInv.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Bundle].Value.ToString())
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Width].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.WidDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        * (clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Length].Value.ToString())
                         + ((clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.LenDec].Value.ToString())) * clsDbManager.ConvDecimal("0.0833")))
                        ;
                ...
                if (str_IsBundle == "0" && str_IsMesh == "0")
                {
                    grdVoucher.Rows[i].Cells[(int)GColCsInv.Amount].Value =
                        clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Rate].Value.ToString())
                        * clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Qty].Value.ToString());
                        //* Convert.ToDecimal(grdVoucher.Rows[i].Cells[(int)GColInv.Bundle].Value);
                }

                //if (grdVoucher.Rows[i].Cells[(int)GColInv.isBundle].Value.GetType = checked)
                //{
                //    bcheck= decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColInv.Rate].Value, out outValue);

                //    grdVoucher.Rows[i].Cells[(int)GColInv.Amount].Value = 
                //        decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColInv.Rate].Value, outValue) 
                //        * decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColInv.Bundle].Value, outValue);
                //}

                if (grdVoucher.Rows[i].Cells[(int)GColCsInv.Amount].Value != null)
                {
                    bcheck = decimal.TryParse(grdVoucher.Rows[i].Cells[(int)GColCsInv.Amount].Value.ToString(), out outValue);
                    if (bcheck)
                    {
                        rtnVal += outValue;
                        fTotalAmount += outValue;
                        fTotalQty += clsDbManager.ConvDecimal(grdVoucher.Rows[i].Cells[(int)GColCsInv.Qty].Value.ToString());
                    }
                }
        ...
        private bool PrepareDocMaster()
        {
            ...
            string str_CashSaleCode = string.Empty;

            try
            {
                string lDocDateStr = StrF01.D2Str(msk_VocDate);
                DateTime lDocDate = DateTime.Parse(lDocDateStr);
                str_CashSaleCode = clsDbManager.GetConfigCode("cmn_Config", "CashSaleID", "", "");
                ...
                if (grdVoucher.Rows.Count > 0)
                {
                    ...
                    ////
                    //lSQL += ", " + msk_AccountID.Tag; //grdVoucher.Rows[dGVRow].Cells[(int)GColInv.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                    ////                                                                                       // 5- Ac Title NA
                ...
                lSQL += ", " + fDocID.ToString();                          // 2- Form 1- Voucher_id
                //
                lSQL += ", " + clsDbManager.ConvInt(str_CashSaleCode); //msk_AccountID.Tag; //grdVoucher.Rows[dGVRow].Cells[(int)GColInv.acid].Value.ToString();            // 2- Serial Order replaced with SNo. 
                //                                                                                       // 5- Ac Title NA
                lSQL += ", '" + StrF01.EnEpos(lblAccountName.Text) + "'";      // 8- Narration 
                lSQL += ", " + 0;           // 9- Debit. 
                lSQL += ", " + clsDbManager.ConvDecimal(lblTotalAmount.Text);           // 10- Credit
        ...
        private bool PrepareDocDetail()
        {
            ...
            //        //
            //        if (grdVoucher.Rows[dGVRow].Cells[(int)GColInv.ItemID].Value == null)
            //        {
            //            if (dGVRow == fLastRow)
            //            {
            //                continue;
            ...
            //        else
            //        {
            //            if ((grdVoucher.Rows[dGVRow].Cells[(int)GColInv.ItemID].Value.ToString()).Trim(' ', '-') == "")
            //            {
            //                //lBlank = true;
            //                if (dGVRow == fLastRow)
            //                {
            ...
            //        lSQL += " WHERE i.goodsitem_id=" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.ItemID].Value.ToString();
            ...
            //        if (grdVoucher.Rows[dGVRow].Cells[(int)GColInv.isMesh].Value != null)
            //        {
            //            if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColInv.isBundle].Value.ToString()) == "0"
            //                || clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColInv.isMesh].Value.ToString()) == "0")
            //            {
            //                strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Bundle].Value.ToString()
            //                    + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Qty].Value.ToString()
            //                    + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Rate].Value.ToString() ;
            //            }

            //            if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColInv.isBundle].Value.ToString()) == "1")
            //            {
            //                strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Bundle].Value.ToString()
            //                    + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Rate].Value.ToString();
            //            }

            //            if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColInv.isMesh].Value.ToString()) == "0")
            //            {
            //                strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Bundle].Value.ToString()
            //                    + " L=" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Length].Value.ToString()
            //                    + " W=" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Width].Value.ToString()
            //                    + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Qty].Value.ToString()
            //                    + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Rate].Value.ToString();
            //            }
            ...
            //        //}
            //        //grdVoucher.Rows[dGVRow].Cells[(int)GColInv.isMesh].Value.ToString()
            ...
            //        lSQL += ", " + fDocID.ToString();                                                        // 2- Form 1- Voucher_id
            //        //
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.ItemID].Value.ToString();            // 2- Serial Order replaced with SNo. 
            //        //                                                                                       // 5- Ac Title NA
            //        lSQL += ", '" + StrF01.EnEpos(grdVoucher.Rows[dGVRow].Cells[(int)GColInv.ItemName].Value.ToString()) + "'";      // 8- Narration 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.UOMID].Value.ToString();            // 2- Serial Order replaced with SNo. 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.GodownID].Value.ToString();            // 2- Serial Order replaced with SNo. 

            //        lSQL += ", 0"; //+ grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Qty].Value.ToString();           // 9- Debit. 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Qty].Value.ToString();           // 9- Debit. 

            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Rate].Value.ToString();          // 10- Debit. 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Bundle].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.MeshTotal].Value.ToString();          // 11- Combo 1 

            //        lSQL += ", " + clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColInv.isBundle]);           // 11- Combo 1 
            //        lSQL += ", " + clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColInv.isMesh]);           // 11- Combo 1 

            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Length].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.LenDec].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.Width].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + grdVoucher.Rows[dGVRow].Cells[(int)GColInv.WidDec].Value.ToString();          // 11- Combo 1 
            //        lSQL += ", " + dGVRow.ToString(); //is Checked
            //        lSQL += ", 0"; //is Checked
            //        lSQL += ")";
        ...
        { 
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < grdVoucher.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            continue;
                    ...
                    {
                        if ((grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
                    ...
                    lSQL = "SELECT gg.Group_st + i.goodsitem_st AS Item_ST ";
                    lSQL += " FROM gds_item i INNER JOIN gds_Group gg ON i.Group_id=gg.Group_id ";
                    lSQL += " WHERE i.goodsitem_id=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.ItemID].Value.ToString();
                    lSQL += "; SELECT goodsuom_st FROM gds_uom WHERE goodsuom_id=";
                    lSQL += grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.UOMID].Value.ToString();
                    ...
                    if (grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.isMesh].Value != null)
                    {
                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.isBundle].Value.ToString()) == "0"
                            && clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.isMesh].Value.ToString()) == "0")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Bundle].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Qty].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.isBundle].Value.ToString()) == "1")
                        {
                            strItemDes += " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Bundle].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Rate].Value.ToString();
                        }

                        if (clsDbManager.ConvBit(grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.isMesh].Value.ToString()) == "1")
                        {
                            strItemDes += " B=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Bundle].Value.ToString()
                                + " x L=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Length].Value.ToString()
                                + " x W=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Width].Value.ToString()
                                + " Q=" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.MeshTotal].Value.ToString()
                                + " " + strUOM_ST
                                + " @" + grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Rate].Value.ToString();
                        }
                    ...
                    //                                                                                       // 5- Ac Title NA
                    lSQL += ", '" + strItemDes + "'";      // 8- Narration 
                    lSQL += ", " + clsDbManager.ConvDecimal(grdVoucher.Rows[dGVRow].Cells[(int)GColCsInv.Amount].Value.ToString());           // 9- Debit. 
                    lSQL += ", " + 0;           // 10- Credit
                    lSQL += ", " + 0;          // 11- Combo 1 
                    lSQL += ", 0"; //is Checked
                    lSQL += ")";
        ...
        private int GridTNOT(DataGridView pdGv)
        {
            ...
            try
            {
                ...
                for (int dGVRow = 0; dGVRow < pdGv.Rows.Count; dGVRow++)
                {
                    ...
                    // Prepare Save Data to Db Table
                    //
                    if (pdGv.Rows[dGVRow].Cells[(int)GColCsInv.ItemID].Value == null)
                    {
                        if (dGVRow == fLastRow)
                        {
                            break;
                    ...
                    {
                        if ((pdGv.Rows[dGVRow].Cells[(int)GColCsInv.ItemID].Value.ToString()).Trim(' ', '-') == "")
                        {
                            //lBlank = true;
                            if (dGVRow == fLastRow)
                            {
        ...
        private void btn_View_Click(object sender, EventArgs e)
        {
            ...
            CrPurchCash rpt1 = new CrPurchCash();
        ...
        private void btn_PinIDAc_Click(object sender, EventArgs e)
        {
            if (btn_PinIDAc.Text == "&Pin")
            {
                btn_PinIDAc.Text = "&Un Pin";
                btn_PinIDAc.Image = Properties.Resources.tiny_pinned;
                msk_AccountID.Enabled = false;
            }
            else
            {
                btn_PinIDAc.Text = "&Pin";
                btn_PinIDAc.Image = Properties.Resources.tiny_pin;
                msk_AccountID.Enabled = true;
            }
        }
